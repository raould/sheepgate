...
import Es from './enemy_small5';
import Em from './enemy_mega5';
import Ehm from './enemy_hypermega5';
...
const LKfn = (level_index: number): Lta.LevelKonfig => {
	...
	Eb4: { mk: Eb4.warpin_mk, count: 4 + buf, limit: 5 + buf, delay_msec: 1000, tick_msec: 3*1000 },
	Eb2: { mk: Eb2.warpin_mk, count: 4 + buf, limit: 5 + buf, delay_msec: 1000, tick_msec: 2*1000 },
	Ep: { mk: Ep.warpin_mk, count: 2 + buf, limit: 4 + buf, delay_msec: 3*1000, tick_msec: 3*1000 },
	Es: { mk: Es.warpin_mk, count: 4 + buf, limit: 2 + buf, delay_msec: 10*1000, tick_msec: 3*1000 },
	Em: { mk: Em.warpin_mk, count: 2 + buf, limit: 1 + buf, delay_msec: 3*1000, tick_msec: 3*1000 },
	Ehm: { mk: Ehm.warpin_mk, count: 1 + buf, limit: 2 + buf, delay_msec: 1000, tick_msec: 5*1000 },
	BG_COLOR: RGBA.new01(0.06, 0.015, 0.1),
